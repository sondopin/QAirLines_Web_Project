...
const flightController = {
    'searchFlights': async (req: Request, res: Response) => {
      try {
        const { ori_airport, des_airport, departure, nums_busi, nums_eco } = req.body;
        ...
        const flights = await Flight.find({
          ...
          actual_departure: { $gte: departureTime },
        });
    ...
    'makeBooking': async (req: Request, res: Response) => {
      try {
        const { flight_id, listInfo, nums_busi, nums_eco, discount_code } = req.body;
        ...
        const businessSeats = await Seat.find({ 
          flight_id, 
          seat_class: "Business", 
          is_available: true 
        }).limit(nums_busi);
    
        const economySeats = await Seat.find({ 
          flight_id, 
          seat_class: "Economy", 
          is_available: true 
        }).limit(nums_eco);
        ...
        let cancellation_deadline = new Date(booking_date.getTime() + 5 * 24 * 60 * 60 * 1000); 

        const max_deadline = new Date(flight.scheduled_departure.getTime() - 1 * 24 * 60 * 60 * 1000);
